  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (data.user) {
        ...
        if (roleError || !roleData || roleData.role !== 'producer') {
          await supabase.auth.signOut();
          throw new Error("Vous n'avez pas de compte producteur");
        }
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#002654] to-[#1E88C7] flex items-center justify-center p-4">
      <div className="bg-white rounded-lg shadow-2xl p-8 w-full max-w-md">
        <div className="text-center mb-8">
          ...
          />
          <h1 className="text-3xl font-bold text-[#002654] mb-2">Espace Producteur</h1>
        ...
        <div className="mt-6 text-center space-y-2">
          <button
            onClick={() => navigate('/producer/forgot-password')}
          ...
          <div className="text-gray-500 text-sm">
            Pas encore inscrit ? <button onClick={() => navigate('/')} className="text-[#1E88C7] hover:underline">Inscrivez-vous</button>
          </div>
        </div>
      </div>
    </div>
